...
UX khuyến nghị: mở 2 terminal — một chạy `/use-contextd "..."`, một chạy `/contextd-viz --watch`. Browser mở sẵn `trace.html`.
...
|---|---|
| Vừa chạy `/use-contextd`, muốn debug 1 run | `/contextd-viz --last` |
| Muốn xem 1 run cũ | `/contextd-viz {run_id}` |
| Muốn so sánh nhiều runs theo workspace/date/verdict | `/contextd-viz --all` |
| Đang chạy `/use-contextd` ở terminal khác, muốn xem live | `/contextd-viz --watch` (terminal mới) |
