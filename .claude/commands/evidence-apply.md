...
Feed `verified-facts.md` (+ optional `10-final-report.md`) vào logic của `/update-contextd` hoặc `/rebase-contextd`. Tạo audit trail tại `applied/{evid-id}/manifest.yaml`.

> Step 4 trong pipeline: ingest → analyze → qa → **apply**.
> Reference: [agents/pipeline/evidence-lifecycle.md](../../agents/pipeline/evidence-lifecycle.md), [update-contextd.md](update-contextd.md), [rebase-contextd.md](rebase-contextd.md).
...
Apply via: /{update-contextd|rebase-contextd} workflow rules.
```

## Bước 5 — Invoke update-contextd / rebase-contextd logic (checkpointed)
...
Run [update-contextd.md](update-contextd.md) **Bước 1–5** với evidence là input chính. Map sub-steps:
...
Run [rebase-contextd.md](rebase-contextd.md) **Bước 1–8** với rule bổ sung:
...
| Sub-step rebase-contextd                                    | checkpoint `step` value          | Resume granularity         |
|---------------------------------------------------------|----------------------------------|----------------------------|
...
Ghi `applied/{id}/diff-summary.md` theo format Bước 8 báo cáo của [rebase-contextd.md](rebase-contextd.md):
```markdown
...
Optional next:
  /evidence-archive --id {id}    → move to archive after some time
                                   (planned — not yet implemented; archive manually for now)
```
